...
type IssueStage = "Submitted" | "Assigned" | "In Progress" | "Resolved";
...
export interface NormalizedIssue extends IssueDto {
  id: string;
  title: string;
  description: string;
  category: string;
...
interface IssuesContextValue {
  ...
  stats: { resolved: number; active: number; pending: number };
  loading: boolean;
  error: string;
  submitting: boolean;
  loadIssues: () => Promise<void>;
  submitIssue: (payload: SubmitIssuePayload) => Promise<NormalizedIssue>;
  findById: (id: string) => NormalizedIssue | undefined;
  loadIssueById: (id: string) => Promise<NormalizedIssue | null>;
}

const IssuesContext = createContext<IssuesContextValue | null>(null);

const orderedStages: IssueStage[] = ["Submitted", "Assigned", "In Progress", "Resolved"];

const statusMap: Record<string, IssueStage | string> = {
  OPEN: "Submitted",
  ASSIGNED: "Assigned",
...
function fallbackTimeline(status?: string, createdAt?: string | number, updatedAt?: string | number): TimelinePoint[] {
  const current = normalizeStatus(status);
  const created = toTimestamp(createdAt);
  const updated = toTimestamp(updatedAt || createdAt);
  const timeline: TimelinePoint[] = [{ key: "Submitted", timestamp: created }];
...
function mapTimeline(
  transparencyData: { timeline?: Array<{ timestamp?: string | number; details?: { newStatus?: string } }> } | null,
  ...
  const points: Partial<Record<IssueStage, number>> = {
  ...
  for (const event of events) {
    ...
    if (!status) continue;
    const mapped = normalizeStatus(status) as IssueStage;
  ...
  return orderedStages.filter((key) => Boolean(points[key])).map((key) => ({ key, timestamp: points[key] || Date.now() }));
}

function normalizeIssue(issue: IssueDto): NormalizedIssue {
  ...
  return {
    ...
    timeline: fallbackTimeline(status, String(createdAt), String(updatedAt)),
...
export function IssuesProvider({ children }: { children: ReactNode }) {
  ...
  const upsertIssue = (incomingIssue: NormalizedIssue) => {
    setIssues((prev) => {
  ...
  const submitIssue = async (payload: SubmitIssuePayload) => {
    setSubmitting(true);
    try {
  ...
  const loadIssueById = async (id: string): Promise<NormalizedIssue | null> => {
  ...
  const findById = (id: string) => issues.find((issue) => issue.id.toLowerCase() === id.toLowerCase());
